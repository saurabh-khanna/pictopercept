...
if "consent_given_jobs" not in st.session_state:
    st.session_state.consent_given_jobs = False

# Consent button
if not st.session_state.consent_given_jobs and not st.session_state.survey_ended:
    st.write("""
            In this survey, you will see pairs of photos and a job title, like "Who of these is a teacher?" or "Who of these is a painter?"
    ...
    if st.button("Let us begin!", type="primary", use_container_width=False):
        st.session_state.consent_given_jobs = True
        st.rerun()


if st.session_state.consent_given_jobs and not st.session_state.survey_ended:

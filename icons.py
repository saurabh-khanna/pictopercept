...
if "consent_given_icons" not in st.session_state:
    st.session_state.consent_given_icons = False

# Consent button
if not st.session_state.consent_given_icons and not st.session_state.survey_ended:
    st.write("""
            In this survey, we show you pictures of two persons at a time, and ask who you might follow on social media. You must choose one person, and their profile picture is the only information you have.
    ...
    if st.button("Let us begin!", type="primary", use_container_width=False):
        st.session_state.consent_given_icons = True
        st.rerun()


if st.session_state.consent_given_icons and not st.session_state.survey_ended:
    ...
    TEXT = "<span style='font-size:24px;'>Who will you **follow** on social media?</span>"
